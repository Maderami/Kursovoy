...
import com.docjpa.laba2.JPA.S.models.ApproveRej;
import com.docjpa.laba2.JPA.S.models.Item;
...
public class AdminController {
    ...
    public String documentConsole(Model model) {
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            int id = listdocrepository.findByName(author.getId_user()).getAuthor().getId_user();
            long count = lisd.stream().filter(x -> x.getAuthor().getId_user() == id).count();
            long countApprove = lisd.stream().filter(x -> (x.getAuthor().getId_user() == id && x.getStatus().equals("Approve"))).count();
            long countReject = lisd.stream().filter(x -> (x.getAuthor().getId_user() == id && x.getStatus().equals("Reject"))).count();
            items.add(i, new Item(author.getFsm_sub(), (int) count, (int) countApprove,(int) countReject));

        }

//        model.addAttribute("userC", users);
//        model.addAttribute("listU", users);
        model.addAttribute("listd", items);
